...
| HEAD | v0.64.0 поверх мержа `15de20f` (main: «add makets») + `ab0e6c3` (tip da2c, v0.63.0+docs); `main` обновлён пользователем до `15de20f` |
| Версия | `0.64.0` (package.json), тег `v0.64.0` запушен |
| Релиз | v0.64.0 собран в CI (release.yml, windows-latest) по тегу; артефакты: `NetMap-Setup-0.64.0.exe`, `NetMap-Portable-0.64.0.exe`, `latest.yml` |
...
Пользователь утвердил план; **выпущен v0.64.0 «точки-клиенты на хабах + тултипы + пинг/SSH
с карты»** (приёмы макета B): секция Connected Devices = сетка интерактивных точек 13 px
(клик/Ctrl+клик/двойной/ПКМ, цвет = тип, красный = down), счётчик «N down» в шапке;
`src/DeviceTooltip.tsx` — единый живой тултип (имя/IP/MAC/статус+RTT) на карточках, маяках
и точках (заменил native title); в ПКМ-меню устройства — «Пинг» (pingBatch + pushAlert)
и «SSH-терминал» (учётка из vault/карточки, событие `netmap:open-ssh-terminal`).
EndpointChip/EndpointRow удалены. Проверки: tsc + vite build + jsdom-стенд 15/15
(/tmp/nmdots, не персистит). Следующий шаг — **v0.65 «иерархия и фокус»** (приёмы A):
ховер-подсветка соседей с затемнением, двойной штрих магистралей, контейнеры подсетей,
связи под карточками на near, кегль 10→11 px. Снапшот в начале сессии сбрасывался —
починено мержем `origin/arena/01a0da2c-netmap` (`--allow-unrelated-histories`).
...
1. **Макеты карты (`map-variants.html`) — гибрид в работе: v0.64.0 ВЫПУЩЕН.**
   Разбор — `docs/map-variants-analysis.md`. План (утверждён пользователем):
   ~~v0.64 — точки-клиенты + ПКМ-меню + тултипы (B)~~ **готово**;
   **v0.65 — «иерархия и фокус» (A): ховер-подсветка соседей с затемнением остальных,
   двойной штрих магистральных связей, контейнеры подсетей (надстроить GroupNode),
   связи под карточками на близком зуме, кегль второстепенных строк 10→11 px**;
   v0.66 — радиальная стратегия раскладки + режим списка в правой панели с синхроном (C/D).
