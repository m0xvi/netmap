...
## v0.51.15 — Компактный импорт и меню без значков

- Ограничен размер окна импорта MikroTik; содержимое прокручивается внутри окна.
- Значки из меню Help удалены полностью, без пустых колонок и лишних отступов.
- Меню корректно поддерживает пункты без иконок.

## v0.51.14 — Исправление окна настроек

- Исправлено дублирование заголовка и пунктов навигации в настройках после замены эмодзи-иконок.
- Иконки и подписи бокового меню снова разделены корректно.
- Сохранён масштаб всего интерфейса, включая portal-окна.

## v0.51.13 — Масштаб интерфейса и меню действий

- Масштаб интерфейса теперь распространяется на весь `body`, включая портальные окна, настройки, импорт и диалоги.
- Убран двойной масштаб основного контейнера.
- Исправлено позиционирование категорий меню действий LayoutFAB.
- Из наиболее заметных меню и настроек убраны несоответствующие интерфейсу эмодзи.

## v0.51.12 — Группы, перемещение и анимации

- Импорт размещает новые группы за пределами текущей карты с учётом реальных границ устройств и групп.
- Исправлено повторное создание одинаковых групп при импорте одной подсети.
- Добавлена группировка импорта по типам устройств.
- Устройства можно перетаскивать из одной группы в другую или за пределы группы.
- В настройки добавлен параметр отключения анимаций на карте.
- FAB-категории позиционируются отдельно от центральной кнопки.

## v0.51.11 — Фиксация viewport и групп импорта

- Карта больше не прокручивает весь интерфейс при движении колеса мыши.
- Исправлено позиционирование категорий LayoutFAB.
- Импорт теперь повторно использует группы, созданные в рамках текущего прохода, поэтому устройства одной подсети попадают в одну группу.

## v0.51.10 — Исправление импорта и LayoutFAB

- Исправлено перекрытие широких категорий LayoutFAB с центральной кнопкой.
- Импорт MikroTik теперь учитывает размещение «Ниже текущей карты» и для групп.
- Убрана автоматическая раскладка после импорта, которая игнорировала выбранное размещение и собирала хосты в строку.
- Группы по подсетям получают детерминированные позиции и не создаются случайно поверх карты.

## v0.51.9 — Упрощение действий и редактирования

- LayoutFAB сгруппирован в три категории: «Раскладка», «История», «Инструменты».
- Основные текстовые поля устройства сохраняются после завершения редактирования, а не на каждую набранную букву.
- Снижен объём лишних history snapshots и persistence-операций при редактировании.

## v0.51.8 — Упрощение навигации по видам карты

- Убраны дублирующие переключатели Modern/Legacy и Compact/Rack из верхней панели.
- Переключение режима карточек и компактности endpoint-ов перенесено в единое меню «Виды».
- Кнопки сохранённых видов, быстрых пресетов и отображения карты теперь находятся в одном месте.
- Панель управления сверху стала компактнее: проект, поиск, виды, уведомления и помощь.

## v0.51.7 — Сохранённые виды карты

- Добавлено меню «Виды» в toolbar.
- Быстрые виды: «Обзор», «Только инфраструктура», «Только камеры», «Только новые».
- Пользовательские виды сохраняют масштаб, позицию viewport и фильтры.
- Виды хранятся отдельно для каждого проекта и доступны после перезапуска приложения.
- Для быстрых видов карта автоматически подстраивается под отфильтрованные устройства.

## v0.51.6 — Bulk Layer и отчёт по портам импорта

- В панель групповых операций добавлено назначение сетевого слоя: Core, Distribution или Access.
- Итог импорта MikroTik теперь показывает количество подключений с назначенным портом и без свободного порта.
- При нехватке портов выводится отдельное предупреждение с рекомендацией проверить связи в инспекторе.

## v0.51.5 — Транзакционный импорт и массовый Location

- Весь импорт MikroTik теперь отменяется одной операцией `Ctrl+Z`.
- В панели групповых операций добавлено назначение `Location` для выбранных устройств.

## v0.51.4 — Автонастройка портов и управление профилями

- Режим «Порт: авто» теперь назначает первый свободный порт выбранного router/switch.
- Автоматически назначенные порты резервируются внутри текущего импорта, чтобы не создать дубликаты.
- При отсутствии свободных портов связь создаётся без конкретного порта для ручной настройки позже.
- Добавлено удаление сохранённых профилей импорта.

## v0.51.3 — Порты и профили импорта MikroTik

- В review импорта добавлен выбор свободного порта выбранного router/switch или автоматическое назначение.
- Настройки импорта можно сохранять в локальные профили без пароля и адреса роутера.
- Профиль сохраняет группировку, имя группы, размещение, поля сопоставления и политику конфликтов.

## v0.51.2 — Удобная настройка подключений при импорте

- Добавлено массовое назначение одного router/switch для всех импортируемых хостов.
- Добавлен быстрый сброс назначенных подключений.
- Можно выбрать размещение новых устройств справа или ниже текущей карты.
- В итоговой строке review показываются совпадения и количество создаваемых подключений.

## v0.51.1 — Упрощённый review импорта MikroTik

- Убрана лишняя мини-карта из окна настройки импорта.
- Выбор существующего router/switch для подключения импортируемых хостов сохранён.

## v0.51.0 — Расширенный импорт MikroTik и масштаб интерфейса

- FAB теперь раскрывает действия ровным горизонтальным рядом влево от главной кнопки.
- В «Настройки → Общие» добавлен масштаб всего интерфейса от 80% до 125%.
- После сканирования MikroTik появился отдельный этап настройки импорта: общая группа или подсети, имя группы, политика совпадений по имени/MAC/IP, выбор существующего router/switch и мини-карта опорных устройств.
- Импортированные хосты раскладываются сеткой рядом с текущей картой и могут сразу получить связь с выбранным router/switch.
