...
## v0.68.0 — Контракт сцены: единые правила LOD, фасовки и обзора больших карт

- Логика отображения вынесена в один чистый модуль `src/scenePlan.ts` («контракт сцены», спецификация — `docs/display-logic.md`): зум меняет детализацию, а не существование — устройство представлено карточкой, маяком или счётчиком в хабе/пилюле, ничто не пропадает.
- Обзор (дальняя ступень): группы — пилюли 44px со счётчиком состава (двойной клик — «нырок» в группу), хабы — маяки, оконечные с одним хабом фасуются в него, параллельные кабели агрегируются в пучки «×N» (`BundleEdge`), магистрали — двойным штрихом. Бюджет обзора: десятки узлов и рёбер вместо тысяч портовых якорей.
- Средняя ступень: карточки, портовые якоря только у выделенного/под курсором, рёбра к боковым якорям по геометрии; ближняя — полная детализация с портами.
- Узлы и рёбра Canvas больше не держат разрозненных эвристик «кого спрятать» — оба мемо берут единый план (один расчёт на изменение входов, с учётом фильтров кабелей/VLAN/слоёв).
- Проверки: tsc + vite build; юнит-тест контракта сцены 17/17 (фасовка оконечных и состава пилюль, транзит в пилюлю, пучки ×N для параллельных кабелей, карточки/рамки на mid/near, collapseEndpoints на mid, уважение фильтров, детерминизм); регрессии: радиальная 8/8, стратегии 29/29.

## v0.67.0 — Режимы раскладки работают на любой карте + оптимизация DOM

- Исправлено «мёртвое» меню стратегий раскладки. Причина: автогруппировка не трогала устройства, уже состоящие в пользовательских группах, — на карте с готовыми группами любая стратегия давала ту же картину. Теперь стратегия переорганизует всю карту (пользовательские группы на время «засыпают», устройства получают автогруппы по локации/VLAN/подсети), а «Разложить заново (без группировки)» возвращает устройства в их домашние группы (поле `userGroupId`, откат также через Ctrl+Z). Пустые «спящие» группы не рисуются рамками.
- Обратная связь: после умной раскладки выводится итог — сколько автогрупп создано и сколько устройств в них, а если данных для группировки нет (нет локаций/VLAN/IP), об этом говорится прямо.
- Оптимизация карты: невидимые портовые якоря (2 DOM-узла на порт, до половины DOM схемы) рендерятся только на ближнем зуме, при выделении или ховере устройства; на средней/дальней ступенях рёбра цепляются к боковым якорям по геометрии направления кабеля. Меньше DOM — быстрее перерисовка и раскладки на больших картах.
- Проверки: tsc + vite build; стенд на реальном store 37/37 (takeover на сгруппированных сид-картах, «none» восстанавливает домашние группы, идемпотентность, конечность координат, радиальная после стратегий).

## v0.66.0 — Радиальная раскладка «радуга» и синхронный список устройств

- Завершение переноса приёмов из макетов (map-variants.html, варианты C и D — разбор в docs/map-variants-analysis.md). Новая радиальная раскладка: ядро сети — в центре, хабы — по орбите вокруг него, оконечные устройства — дугами вокруг своего хаба (наружу от центра). Для отеля со звездой «роутер → этажные свитчи → камеры/AP/ПК» читается лучше любой иерархии. Группы остаются рамками (состав раскладывается сеткой внутри), несвязанные устройства — ряд под схемой. Раскладка детерминирована, отменяется через Ctrl+Z, координаты проходят safeFinite-контроль.
- Пункт «Радиальная · «радуга»» добавлен в меню стратегий умной раскладки (шеврон на панели инструментов, под разделителем — это геометрия, а не группировка) и в меню Вид → «Радиальная раскладка (ядро в центре)».
- В инспекторе устройства (вкладка «Инфо») появился раздел «Подключённые устройства» с двусторонним синхроном «список ↔ карта»: наведение на строку затемняет на схеме всё, кроме этого устройства и его соседей, и подсвечивает его связи; наведение на карточку на карте подсвечивает строку и прокручивает её в видимую область. Клик по строке — выбрать устройство, двойной клик — крупный вид. В шапке раздела — общий счётчик и красный счётчик «N down»; сортировка: инфраструктура сверху, оконечные по типам.
- Затемнение не-соседей при наведении (v0.65) переведено на единый источник — store.hoveredDeviceId: теперь его одинаково вызывают и карта, и список в панели.
- Проверки: tsc + vite build; юнит-тест радиальной геометрии 16/16 (ядро в центре, хабы на орбите 380, дуги оконечных 110 px наружу от хаба, группа-юнит на орбите с относительными координатами детей, сироты, детерминизм, пустой док, конечность координат) + jsdom-стенд списка 9/9 (строки и счётчики, сортировка, ховер в обе стороны, подсветка, клик/двойной клик, IP и тип-бейдж).

## v0.65.0 — Иерархия и фокус: соседи, магистрали, контейнеры, связи под карточками

- Второй шаг переноса приёмов из макетов (map-variants.html, вариант A — «Контейнеры»). Наведение на устройство затемняет все несвязанные карточки (остаются «в фокусе» только сам хаб и его соседи по кабелям), связи при этом гаснут как и раньше — вся картина «что куда подключено» читается одним наведением. Затемнение плавное (150 мс), работает без перерисовок React, сбрасывается при уходе курсора, драге и клике мимо; уважает тумблер «Фокус связанных при hover».
- Магистральные связи (хаб↔хаб: роутер/свитч/патч-панель/сервер/облако/VPS) рисуются двойным штрихом: толстая полупрозрачная подложка + обычная сердцевина. Ядро и аплинки теперь видны с первого взгляда; подложка не мешает кликам и «ножу».
- Группы-контейнеры приведены к виду макета: спокойная сплошная граница 35% вместо кричащей пунктирной, заливка 5%, в шапке — цветная точка-маркер, жирное имя и счётчик устройств в пилюле. Выделенная группа — граница полного цвета.
- На близком зуме (near) связи уходят под карточки — кабели больше не режут текст на карточках. На среднем/дальнем зуме и в режиме «нож» связи по-прежнему поверх (обзор и резка не пострадали).
- Кегль второстепенных строк карточек поднят с 10 до 11 px (модель · IP у хабов, IP у оконечных) — читается на 1080p без напряжения.
- Проверки: tsc + vite build, jsdom-стенд 13/13 (стили контейнера в обычном и выделенном состоянии, точка-маркер и счётчик, подложка магистрали и её толщина/некликабельность, отсутствие подложки у обычных связей, сохранение hit-дорожки, кегли 11 px хаба и листа).

## v0.64.0 — Точки-клиенты на хабах, живые тултипы, пинг и SSH с карты

- Первый шаг переноса приёмов из макетов (map-variants.html, разбор — docs/map-variants-analysis.md). Секция «Connected Devices» на карточке хаба вместо аккордеона списков теперь рисует сетку интерактивных точек: каждая точка — одно свёрнутое оконечное устройство. Цвет = тип (камера/AP/ПК/…), красный = недоступен, полупрозрачные = нет данных. Точки видны и на среднем зуме — раньше там были только счётчики.
- Точка полноценно интерактивна: клик — выбрать (правая панель), Ctrl+клик — добавить к мульти-выделению (ранее выбранное устройство подхватывается в набор), двойной клик — крупный вид, правый клик — меню устройства. Кликабельная цель 13×13 px вместо невидимых портовых якорей 6×6.
- Шапка секции показывает общий счётчик и красный счётчик «N down» — недоступные оконечные видны без раскрытия.
- Живой тултип устройства (имя, тип, IP, MAC, модель, статус + RTT) на карточках хабов, оконечных, «маяках» и точках: один плавающий элемент на всю карту, статус обновляется во время ховера; заменил нативные title-подсказки.
- В контекстное меню устройства добавлены «Пинг» (разовая проверка с результатом в статус и уведомления: RTT или таймаут) и «SSH-терминал» (учётные данные берутся из связи с vault или карточки устройства).
- DOM-экономия: карточка хаба с 26 оконечными — ~40 узлов вместо ~160 (аккордеон + строки); каждая точка подписана только на своё устройство, тик мониторинга перерисовывает одну точку.
- Проверки: tsc + vite build, jsdom-стенд 15/15 (рендер точек и счётчиков, клик/Ctrl+клик/двойной клик/ПКМ, кольцо выделения, тултип имя/IP/MAC/статус и скрытие, видимость точек на mid, «маяк» на far).

## v0.63.0 — Плавающая кнопка удалена, стратегии раскладки — на полосе

- Синий круг LayoutFAB (правый верхний угол карты) удалён: он дублировал панель инструментов под тулбаром — те же отмена/повтор, нож, связи, раскладка, развернуть/свернуть, экспорт PNG/SVG/JSON. Карта стала чище, из кадра ушёл лишний плавающий элемент (вместе с ним — всплывающая подсказка «схема выглядит запутанно»).
- Единственное, что было только на удалённой кнопке — выбор стратегии умной раскладки. Он переехал на полосу: «Умная раскладка» стала split-кнопкой. Клик по иконке — гибрид (как раньше, в один клик), клик по шеврону — меню из 4 стратегий: гибрид (локация → VLAN → подсеть /24), только по локациям, только по VLAN, только по подсети /24. Классическая раскладка без группировки — соседняя кнопка.
- Меню закрывается по Escape, клику мимо и при скролле/сворачивании полосы; кегль/отступы в стиле полосы. Подсказки «?» в тулбаре обновлены (раздел «Раскладка схемы» вместо «Плавающая кнопка»), устаревшие упоминания кнопки вычищены из кода и советов.
- Runtime-проверка jsdom: 23/23 (полоса, split-кнопка, все 4 стратегии, закрытие меню, сворачивание полосы без «залипшего» меню). Бандл стал на ~9 КБ меньше.

## v0.62.2 — Тихая фоновая проверка обновлений

- Красная плашка ERR_TIMED_OUT на старте больше не пугает: сетевой сбой фоновой проверки (GitHub недотянулся) уходит молча в консоль. Плашка + алерт остались для ручной проверки из меню и не-сетевых ошибок (404/403/конфиг).
- Апдейтер помечает происхождение проверки (auto/manual), баннер распознаёт Chromium-ошибки сети (ERR_TIMED_OUT, ERR_INTERNET_DISCONNECTED и др.) и для ручной проверки показывает понятный текст вместо куска стека.

## v0.62.1 — Проверка v0.62.0: 50 runtime-тестов, два фикса

- Контрольный проход по единой теме окон: jsdom-стенд (каркас, все 15 иконок, кнопки, prompt/confirm/alert, 5 диалогов целиком) — 50/50 зелёных. Статика: все классы/иконки на месте, тема есть в прод-бандле.
- Фикс 1: openPortPicker повторяет попытку до ~30 кадров (на тяжёлой карте Host может не успеть за один rAF и окно молча не открывалось); если Host не встал — честный null вместо вечного зависания.
- Фикс 2: у настроек убран свой дублирующий обработчик Escape (теперь только от DialogShell).

## v0.62.0 — Единый стиль всех окон по шаблону автообнаружения

- Новый модуль DialogTheme: общий каркас (бэкдроп, карточка, шапка с градиентной иконкой, серое прокручиваемое тело, фиксированный футер), кнопки primary/ghost/danger, сегмент-контрол, поля, тумблеры, чекбоксы, чипы, теги, инфобоксы — значения из редизайна discovery v0.56.0.
- На каркас переведены: импорты (обычный, MikroTik + его окно настроек), Vault Studio, Vault-импорт/экспорт, traceroute, настройки, выбор портов, QR-шеринг, SSH-терминал, резервные копии, prompt/confirm/alert. Esc и клик по фону закрывают везде одинаково (у терминала и Vault своя логика Esc сохранена).
- Единый zIndex: обычные окна 9500, окна поверх других 99900, prompt/confirm/alert 100000. Онбординг остался стартовым туром поверх всего, но кнопки/шрифты/подложки приведены к теме.

## v0.61.1 — Панель инструментов реально появляется

- Исправление v0.61.0: полоса ToolsStrip была написана, но тег не попал в дерево App (потерялся при параллельных правках) — теперь панель видна под тулбаром. Проверены все 17 кнопок: события fit/discovery/импорты/traceroute/Vault доходят до существующих слушателей, undo/redo/нож/связи/раскладки/экспорт идут через те же store-функции, что FAB и меню.

## v0.61.0 — Панель инструментов в стиле draw.io, легенда подсетей слева

- Новая горизонтальная панель инструментов под тулбаром (на всю ширину): История (отмена/повтор) · Карта (вписать, нож, связи) · Раскладка (умная-гибрид, сверху-вниз, развернуть/свернуть свитчи) · Данные (discovery, MikroTik, импорт, traceroute, Vault) · Экспорт (PNG/SVG/JSON). Иконки + русские тултипы, тумблеры подсвечиваются, логика та же, что в FAB/меню.
- Панель сворачивается шевроном справа (остаётся slim-полоса «Инструменты»), состояние переживает перезапуск; тумблер продублирован в меню Вид → Панели.
- Легенда подсетей переехала из правого верхнего угла (налезала на синюю кнопку и шторку) в левый верхний — там свободно.

## v0.60.0 — Связи в цветах подсетей, пилюли всем хабам, антиперекрытие

- Связи красятся по подсети (/24): стабильная палитра на 12 цветов, назначение по сортировке (не прыгает между рендерами). Подсеть линка берётся со стороны оконечного (у свитчей часто mgmt-IP из другой подсети); межподсетевые и неизвестные остаются цветом кабеля/скорости. Приоритет: ручной цвет линка → подсеть → скорость → кабель. Тумблер «Красить связи по подсетям» в меню Вид и File (персистится, по умолчанию вкл), справа сверху — сворачиваемая легенда.
- Пилюли-ориентиры теперь для ВСЕХ хабов (раньше access-свитчи оставались безымянными точками), лимит поднят до 60. Антиперекрытие: пилюли идут в порядке приоритета (ядро → группы → хабы), налезшие прячутся — как в настоящих картах.
- Тултип пилюли показывает раскладку: «SW-1 — PC 20 · CCTV 5 (клик: приблизиться)». Маяки увеличены ×1.3.
- Про «паровозик» из v0.59: код на месте и проверен — он едет только за СКРЫТЫМИ оконечными (far-ступень или включенная свертка); в разложенном виде видимые ноды двигаются только сами — так задумано.

## v0.59.0 — Куда делись оконечные: чип свертки, долёт к контенту, драг паровозиком

- «124 добавлено, на карте 15»: это свертка collapseEndpoints (по умолчанию вкл) — оконечные сидят в «Connected Devices» хабов. Теперь сверху висит честный чип «В хабы свернуто: N» с кнопкой «Развернуть всё» (тумблер из недр меню больше искать не надо). В far-ступени чип прячется — там уже есть свой про свертку.
- «Приблизить ничего не меняет»: кнопка зумила в центр экрана, а у разреженной карты там пустота. Теперь долетает к ЦЕНТРУ КОНТЕНТА (bbox устройств и групп, с учётом вложенности) на зум 0.75.
- Драг хаба тащит свернутых оконечных: сдвиг коммитится в конце драга (там же, где позиция хаба) — дешево и без пер-кадрового чурна. Работает и в far, и при collapse в любом зуме; драг группы тащит свернутых детей; защита от двойного сдвига при мультиселекте; спрятанные фильтрами не трогаем.

## v0.58.0 — Пилюли-ориентиры: дальний зум читается всегда

- Исправление v0.57.0: «маяки» живут в мировых координатах и при экстремальном отдалении (fit огромной карты, zoom ~0.05–0.1) всё равно сжимались в точки — физику не обмануть. Поэтому добавлен второй слой — пилюли-ориентиры в ЭКРАННЫХ координатах: фиксированный размер при любом зуме.
- Пилюли показывают ядро (роутеры, синяя рамка), группы (пунктир в цвете группы, по центру сверху) и серверы — с именами и счётчиком («R1-CORE · 45» = оконечных за хабом, у групп — число устройств). Лимит 40, приоритет: ядро → группы → серверы.
- Клик по пилюле — плавный долёт к объекту (fitView). Пилюли за экраном скрываются, свёрнутые группы представляют детей одной пилюлей, фильтры видов учитываются.
- Перф: список мемоизирован, позиции обновляются императивно из onMove (прямые style.transform, без ре-рендеров React на каждый кадр pan/zoom).
- Работает и в legacy-виде (слою всё равно, какие ноды под ним).

## v0.57.0 — Обзорная схема: семантический зум карты

- Дальний зум больше не каша: вид карты сам упрощается по трём ступеням (near ≥0.7 / mid 0.35–0.7 / far <0.35, гистерезис против дёрганья на границе). Ручные сворачивания при этом не трогаем.
- Far (<0.35) — «обзорная схема»: оконечные прячутся в хабы (независимо от тумблера), остаются роутеры/свитчи/серверы в виде крупных «маяков» — большое имя, статус, полоса агрегации (иконка типа + счётчик: ПК, камеры, принтеры, AP, VM), магистрали утолщены, лейблы кабелей скрыты.
- Ядро подсвечено: устройства слоя core (роутеры/cloud) — с синим кольцом, свечением и плашкой CORE; видно издалека, где центр сети.
- Mid — компактные карточки (листы без IP-строки, у хабов только шапка Connected Devices с итогом); near — без изменений.
- Внизу всплывает чип «Обзорная схема — оконечные свернуты в хабы» с кнопкой «Приблизить».
- Перф: ступень пишется в стор только в момент смены (не на каждый кадр зума), подписки узкие; legacy-вид не затронут.

## v0.56.0 — Редизайн окна проверки discovery по макету

- Новое окно проверки: шапка с методами, саммари-блок с кликабельными шагами (прокрутка к фильтрам/устройствам/кнопке), 7 живых метрик движка (LLDP, соседи MT, FDB, ARP, DHCP, SNMP с полосой, время).
- Плотная таблица устройств: 7 колонок (галка, тип-селектор, имя, IP, MAC, метки, источник), сортировка по типу/имени/IP, липкая шапка и липкие заголовки групп, клик по строке выбирает; устройства без IP живут внутри группы «Тип не определён» с disabled-галкой.
- Таблица связей: пара устройств + порты «A → B» + источник; поиск теперь фильтрует и связи, скрытые поиском связи не применяются.
- Фильтры: поиск с крестиком, тумблер «Без IP», мастер-галка с indeterminate, чипы подсетей с сегментами и счётчиками, чипы VLAN (+ «Все/Ни одному»), пустые подсети — счётчиком.
- SNMP-тишина и предупреждения — аккордеонами; футер со счётчиками «N устройств · M связей · пропустится · вне фильтра».
- Зумоустойчивость: липкие отступы замеряются ResizeObserver'ом в рантайме, таблицы на minmax+ellipsis без горизонтального скролла, под 760px прячутся вторичные колонки.

## v0.55.0 — Скорость на больших картах, честная раскладка, Vault-порты, review понятнее

- Скорость: каждый узел Modern-вида был подписан на ВЕСЬ документ и перерисовывался по любому чиху — теперь узкие подписки (ссылки/соседи хаба через useShallow, строки по id); все типы узлов мемоизированы с компаратором (device/group/highlighted) — hover и тики мониторинга больше не перерисовывают 200+ карточек.
- Раскладка без налезаний: пачки endpoint'ов под якорями упаковываются последовательно в непересекающиеся слоты (раньше центрировались и налезали); раскладка знает modern-размеры хабов (300px + секция Connected Devices по числу endpoint'ов); свёрнутые endpoint'ы исключаются из раскладки, как с канваса.
- VLAN из discovery теперь пишется в порт устройства — smart-раскладка группирует по VLAN, а не только по /24; после применения карта сама вписывается в экран (fit-view).
- Vault-пикер: порт и логин видны в строках (`192.168.11.1:2222 · admin`) — записи «с портом/без» различимы; удаление записи прямо из пикера; порт попадает в имя новых записей; кнопка «Открыть Vault Studio» для полного управления.
- Review discovery нагляднее: баннер итога («Найдено новых: N устройств · M связей») + 3 нумерованных шага; SNMP-тишина отделена от настоящих предупреждений спокойным синим блоком с советом про галку свипа; после импорта 60+ устройств предлагается кнопка «Свернуть endpoint'ы в свитчи».
- Меню: убран резерв колонки слева (галочка рисуется только по факту, без отступа); в настройках убраны буллеты-иконки вкладок.

## v0.54.0 — Новые типы (АТС/DVR/Другое), чистые меню, масштаб 200%, редизайн review

- Новые типы устройств: АТС (PBX, иконка-телефон), видеорегистратор (DVR/NVR — повышен из server+payload до полноценного типа), «Другое» (прочее/неопознанное, пунктирная иконка с «?»). Везде: палитры, каталог, слои, пресеты (Data + АТС/DVR/Другое, CCTV + регистраторы), редактор железа DVR теперь и для kind=dvr, пресеты регистраторов в сайдбаре создают kind=dvr.
- Автоопределение типа в discovery знает новые типы: «АТС офис», pbx1/ats-2, Asterisk, «Регистратор склад», nvr2, TRASSIR, Dahua NVR в descr → dvr/pbx; неуверенный результат теперь «Другое», а не ПК.
- Верхние меню (File/View/Tools/Monitor + ProjectMenu) без иконок-глифов, как Help: декоративные символы убраны, состояния (галочки/радио) рисуются SVG-галочкой в отдельной колонке; убран двойной разделитель в View.
- Масштаб интерфейса — до 200% (было 125%): слайдер, пресеты 80/100/125/150/200%, лимиты в настройках и App.
- SNMP-статистика честная: «SNMP ответили: 2 из 207» (раньше счётчик попыток выдавался за хосты). Сводное предупреждение о таймаутах объясняет причину прямо в строке: нет агента / другой community / firewall, молчуны пропускаются — это норма, а не ошибка сканирования.
- Широкие подсети роутера (/16 и крупнее) раскрываются по /24 дочерними чипами (если внутри ≥2 разных /24): видно внутренние сети, каждую можно исключить точечно; при исключённом родителе дочерние блокируются.
- Редизайн окна проверки discovery: диалог шире (880), липкая панель фильтров (поиск + «Все видимые (N)»), футер с «Применить» всегда виден, статистика — компактные пилюли в один ряд, в строках моноширинные IP/MAC и мини-чип VLAN.

## v0.53.0 — Редизайн слоёв, порт SSH в Vault, типы устройств и покрытие VLAN

- Панель «Слои / фильтры» перерисована: минималистичные карточки, SVG-иконки пресетов/глаз-переключателей вместо текстовых глифов, складные секции, пилюли-счётчики (у кабелей — счётчики связей), переключатель PoE, пустые секции скрываются целиком.
- Vault сохраняет и подставляет порт SSH (и в discovery, и в обычном импорте, если у вендора есть порт). Исправлен баг подборщика: отсутствующее поле больше не подменяется паролем (так порт и затирался).
- Переименование в предпросмотре discovery стало очевидным: при наведении на строку поле обводится рамкой и появляется кнопка-карандаш (фокус + выделение имени).
- Тип устройства меняется селектором прямо в строке. Бэкенд определяет тип по отпечаткам: имя (вкл. русское: «Камера склад», «коммутатор», sw1/ap2/cam3), OUI MAC, модель в descr (починен баг «RouterBOARD → всегда роутер»: hAP/cAP → AP, CRS → switch), имя VLAN (CCTV/печать/гости).
- Неопознанные по типу уходят в отдельную группу «Тип не определён — выберите тип» с приоритетом над именем; вендор тоже доопределяется по OUI (камеры получают Hikvision/Dahua/Axis).
- Покрытие VLAN расширено: + switch-chip таблицы MikroTik (`/interface ethernet switch vlan/port`, access-порты по always-strip), + статические VLAN-таблицы опрошенных коммутаторов (dot1qVlanStatic — видны даже пустые VLAN), соседи на access-портах тоже получают VLAN. Пустые подсети (/32 PPPoE-хвосты) скрыты из чипов фильтров.
- Имена локальных LLDP-портов: фолбэк на lldpLocPortDesc, если IF-MIB пуст (меньше «port 0» в связях).

## v0.52.0 — Discovery: имена из DHCP, обязательный IP, фильтры и переименование

- Имена устройств: читаем DHCP leases с MikroTik. Приоритет — комментарий лизы (его правит админ), затем собственное имя устройства (LLDP sysName / identity), затем host-name из DHCP, затем IP/MAC-заглушки. У каждой строки бейдж источника имени.
- Устройство без IP добавить нельзя: такие (только MAC) показываются отдельной секцией «Без IP — добавлены не будут» с выключенными галочками, их связи пропускаются со счётчиком.
- Устройства без имени вынесены в отдельную секцию «Без имени — задайте имена»; имя правится прямо в строке предпросмотра, связи и применение подхватывают правку.
- Фильтры как в обычном импорте: поиск по имени/IP/MAC/DHCP, чипы исключения подсетей (эталонные CIDR из /ip/address роутера + авто-/24) и VLAN; скрытое фильтрами в применение не попадает («N вне фильтра»).
- VLAN устройств: MikroTik — по bridge vlan table и vlan-интерфейсам, SNMP — через Q-BRIDGE-MIB (с откатом на BRIDGE-MIB). VLAN пишется тегом «VLAN N» на созданных устройствах.
- LLDP-соседи получают IP из lldpRemManAddr (раньше были без IP); заодно починен разбор management-адресов — рекурсия v0.51.20 теперь реально получает топливо. SNMP-режим дополнительно читает ARP-таблицу (IP-MIB), чтобы эндпоинты из FDB получали IP.
- Умный дедуп между источниками: устройство, встреченное сначала по MAC (FDB), а потом по IP/имени (LLDP/сосед), обогащается, а не двоится.

## v0.51.23 — Режим «оба»: LLDP важнее SSH-дублей, предупреждения сводятся в строки

- В режиме «оба» грубые связи «MikroTik /ip neighbor» больше не вытесняют LLDP-связи по той же паре устройств: если LLDP дал порты с обеих сторон, SSH-дубль убирается — предпросмотр выглядит так же богато, как при SNMP-скане.
- Сотни одинаковых «SNMP probe failed: Request timed out» схлопываются в сводные строки вида «N хостов: … (например: 10.11.5.97, …)» — предупреждения больше не затапливают предпросмотр.

## v0.51.22 — Читаемые кнопки Vault, инлайн-разблокировка, авто-раскладка и perf-режим

- Кнопки «Из Vault» / «В Vault» получили SVG-иконки вместо символа «▣», который на части систем рендерился квадратом.
- Нажатие «Из Vault»/«В Vault» на заблокированном хранилище сразу открывает окно разблокировки с мастер-паролем и продолжает действие после ввода — без похода в другое место.
- После применения автообнаружения карта автоматически раскладывается умной раскладкой (локация/VLAN/подсеть) — сотни новых карточек больше не налезают на группы; откат — два Ctrl+Z.
- Оптимизация больших схем (>150 устройств): рендер только видимых элементов, миникарта отключается, обходной маршрутизатор кабелей заменяется простыми путями — карта перестаёт лагать после автообнаружения.

## v0.51.21 — Vault в формах учётных данных + починка SSH-сбора

- Исправлена ошибка «MikroTik SSH: mt.runCommand is not a function» в автообнаружении: SSH-сбор снова работает (runCommand экспортируется из mikrotik-ssh).
- Во всех формах учётных данных (автообнаружение, импорт по вендорам) появились кнопки «▣ Из Vault» и «▣ В Vault»: подстановка существующей записи и сохранение новой.
- Записи категоризируются: теги назначения (`ssh` / `snmp` / `api`), службы (`mikrotik ssh`, `snmp community`…), хост в `url`, привязка к устройству схемы; подборщик показывает сначала релевантные записи.
- Сохранение учётки MikroTik из импорта теперь тоже пишет теги категоризации; папки «MikroTik»/«SNMP» совместимы со старыми подборщиками.

## v0.51.20 — Рекурсивное обнаружение сети и индикатор «связи скрыты»

- Мастер «Автообнаружение топологии» (меню Tools) получил рекурсивный обход: management-IP LLDP-соседей становятся целями следующих волн SNMP-опроса. Глубина 1–3 прыжка, лимит 24 хоста — вся сеть опрашивается от одного seed-адреса.
- В форме сканирования: галочка «Рекурсивный обход по LLDP» и выбор глубины; в результатах — счётчик реально опрошенных SNMP-хостов.
- Если в проекте есть связи, но на канвасе не видно ни одной, появляется заметный индикатор «Все связи скрыты» / «Связи скрыты фильтрами» с кнопкой возврата в один клик — ситуация «связи пропали» после случайного «Скрыть все связи» (тумблер сохраняется между запусками) больше не загадка.

## v0.51.19 — Контекстное меню смены группы в точке дропа

- Подтверждение смены группы теперь открывается контекстным меню прямо там, куда отпустили устройство: строка-вопрос «Убрать из группы „…“?» / «Переместить в группу „…“?», ниже «Да» и «Отмена».
- «Отмена», Escape или клик мимо возвращают карточку на исходное место.
- Кнопки панели множественного выделения больше не переносятся по словам («В группу», «↯ PoE» — в одну строку).

## v0.51.18 — Подтверждение смены группы перетаскиванием

- Устройство вытащили за пределы его группы — диалог «Убрать из группы „…“?» [Да / Отмена].
- Устройство перетащили в другую группу — диалог «Переместить в группу „…“?» [Да / Отмена].
- При «Отмена» карточка возвращается на исходное место.
- Не тронуты: перетаскивание устройства на устройство (создание связи) и добавление в группу устройства без группы.
- Панель множественного выделения: подписи переведены на русский («Локация», «Слой», «↯ PoE» вместо «Location», «Layer», «Toggle PoE»).

## v0.51.17 — Анимации сворачивания панелей

- Левая и правая панели теперь плавно выезжают и прячутся (слайд 240 мс) вместо мгновенного появления/исчезновения.
- Анимация работает и для внутренних сворачиваний: левая панель до иконочной рейки, правая — до узкой полоски.
- Ширина контента измеряется автоматически — смена содержимого правой панели (обзор ↔ инспектор) не ломает анимацию.
- Анимация отключается системной настройкой «уменьшить движение» и переключателем «Отключить анимации» в настройках приложения.

## v0.51.16 — Аудит интерфейса и логики

- Масштаб интерфейса в десктоп-версии теперь нативный масштаб страницы (через `webFrame`): канвас больше не рассинхронизируется с курсором, а интерфейс не отсекается снизу/справа при масштабе ≠ 100%. В браузерном режиме обрезание `100vh` исправлено компенсацией размера контейнера.
- Некорректное сохранённое значение масштаба (пустое/`"%"`) больше не превращает интерфейс в пустой экран — возвращается 100%.
- Кнопки меню действий (раскладка/история/инструменты) больше не налезают друг на друга и не обрезаются краем карты; подменю стратегий раскладки открывается вверх, не перекрывая ряд.
- Горячая клавиша поиска перенесена с Ctrl+K на Ctrl+F; Ctrl+K освобождён для Vault Studio. Раздел справки обновлён.
- Исправлен цветовой режим канваса: фон панели управления и рамки выделения больше не темнеют на светлой теме.
- Баннер обновления больше не использует системные диалоги (зависали в Electron) — только внутренние окна.
- Импорт схемы и восстановление из файла/бэкапа теперь откатываются одним нажатием «Отменить».
- Импорт MikroTik: убрано ложное сообщение «Схема автоматически разложена», форма сетки рассчитывается по реально выбранным строкам, исправлена нестабильность селектора списка VLAN.
- Восстановление бэкапа «Заменить текущий» честно предупреждает, что из копии создаётся новый проект (оригинал остаётся в списке).
- Подтверждение массового удаления устройств больше не пугает «отменить нельзя» — удаление откатывается через Ctrl+Z.
- Настройки безопасности: исправлена сломанная фраза статуса, убраны эмодзи.
- Панель обзора сети: полосы утилизации больше не скачут случайно при каждом обновлении карты.
- Тотальная зачистка эмодзи во всём интерфейсе (~30 файлов): заменены типографскими символами и SVG по правилам кодовой базы.
